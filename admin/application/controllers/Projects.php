...
class Projects extends MY_Controller
{
	...
	public function approve($project_id = NULL)
	{
		process_login();
		...
		$this->view_data['title'] = "Approve Project";
		$this->view_data['success'] = $this->session->flashdata('success');
		$this->view_data['error'] = $this->session->flashdata('error');
		...
		if ($this->input->post()) {
			$this->form_validation->set_rules('ocaa_number', 'OCAA Number', 'trim|min_length[1]|max_length[150]');
			$this->form_validation->set_rules('lca_number', 'LCA Number', 'trim|min_length[1]|max_length[150]');

			if ($this->form_validation->run()) {

				$project_data = array(
					'ocaa_number' => set_value('ocaa_number') ? set_value('ocaa_number') : NULL,
					'lca_number' => set_value('lca_number') ? set_value('lca_number') : NULL,
				);
				...
				if ($this->projects_model->update_row(array(
					...
					$this->session->set_flashdata('success', 'Project approved successfully.');
					redirect('projects');
				}
			}
		}

		$this->_view('projects/approve');
		$this->_view('projects/approve_js', 82);
		$this->_view('projects/styles', 21);
		$this->_view('projects/js', 81);
		$this->_render();
	}
